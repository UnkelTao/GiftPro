...
$.idealforms.errors = {
  ...
  name: '只允许中文字母数字下划线，长度为3-12.',
  xieyi2: '请点击协议同意该协议',
  username: '字母数字或者下划线，长度为4-12之间',
  pass: '字母数字或者下划线或者某些字符，长度为6-16之间,不允许引号等特殊字符，可以使用@.#%&!',
  repass: '两次密码不一样',
...
var getFilters = function() {
  ...
  var filters = {
    ...
    name: {
      regex: /^[A-Za-z0-9_\u4E00-\u9FFF]{3,12}$/,
      error: $.idealforms.errors.name
    },

    username: {
      regex: /^[A-Za-z0-9_]{3,12}$/,
      error: $.idealforms.errors.username
    },
    xieyi2:{
       regex:/^[a-z]{1,4}$/,
       error:$.idealforms.errors.xieyi2
    },
    pass: {
...
var _defaults = {
  ...
  customFlags: {},
  globalFlags: '',
  onSuccess: function(e) { alert('即将提交.....') },
  onFail: function() { alert('  项必填未填写！') },
  onXieyi: function(){ alert('请阅读协议，并且同意！')},
  responsiveAt: 'auto',
  disableCustom: ''
}
...
$.extend( IdealForms.prototype, {
      ...
      submit: function( e ) {
        ...
        if ( !self.isValid() ) {
          ...
          self.focusFirstInvalid()
           ...
           if(document.getElementById('xieyi2').value==0){
                e.preventDefault()
                o.onXieyi()
                //self.focusFirstInvalid()
                self.lastTab()
  ...
  _getFormElements: function() {
    return {
      ...
      text: this.$form.find('input:not([type="checkbox"], [type="radio"], [type="submit"]), hidden,textarea'),
...
$.extend( IdealForms.prototype, {
  ...
  focusSecPage:function( ){
    var tableName = "Step 2"
